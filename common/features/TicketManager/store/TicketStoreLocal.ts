...
import { type TicketEventEmitter, TicketEventEmitterLocal } from '@common/features/TicketManager/events';
import { type TicketStore } from './TicketStore';

/**
 * Local storage implementation of the ticket store
 * Uses localStorage for persistence and is useful for testing and development
 */
export class TicketStoreLocal implements TicketStore {
  private tickets: Ticket[] = [];
  private readonly storageKey: string = 'ticketStore-tickets';
  private readonly eventEmitter: TicketEventEmitter;

  constructor() {
    this.eventEmitter = new TicketEventEmitterLocal();
  ...
  create(ticket: Ticket): Ticket {
    ...
    console.debug('TicketStoreLocal: Created ticket', newTicket.id);
  ...
  getAllByStatus(ticketStatuses: TicketStatus | TicketStatus[], size: number, offset: number = 0): Ticket[] {
    ...
    console.debug(`TicketStoreLocal: Found ${paginatedTickets.length} tickets with status '${statusArray.join(', ')}' (${offset}-${offset + size - 1} of ${filteredTickets.length})`);
  ...
  get(ticketId: string): Ticket | null {
    ...
    if (ticket) {
      console.debug('TicketStoreLocal: Found ticket', ticketId);
      return { ...ticket }; // Return a copy to prevent external mutations
    } else {
      console.debug('TicketStoreLocal: Ticket not found', ticketId);
      return null;
    }
  }
  ...
  update(ticketId: string, updatedTicket: Ticket): Ticket {
    ...
    console.debug('TicketStoreLocal: Updated ticket', ticketId);
  ...
  getAllEngineerTicketsByStatus(
    ...
    console.debug(`TicketStoreLocal: Found ${paginatedTickets.length} tickets with status '${statusArray.join(', ')}' for engineer '${engineerProfile.name}' (${offset}-${offset + size - 1} of ${filteredTickets.length})`);
    return paginatedTickets;
  }
  ...
  getActiveTicketByCustomer(customerId: string): Ticket | null {
    ...
    if (activeTicket) {
      console.debug('TicketStoreLocal: Found active ticket for customer', customerId, activeTicket.id);
      return { ...activeTicket }; // Return a copy to prevent external mutations
    } else {
      console.debug('TicketStoreLocal: No active ticket found for customer', customerId);
      return null;
    }
  }
   ...
   * If no data exists, initializes with empty array
   */
  private loadTicketsFromStorage(): void {
    try {
      ...
      if (stored) {
        ...
        console.debug('TicketStoreLocal: Loaded tickets from localStorage');
      } else {
        // Initialize with empty array if no stored data exists
        this.tickets = [];
        this.saveTicketsToStorage();
        console.debug('TicketStoreLocal: Initialized with empty array and saved to localStorage');
      }
    } catch (error) {
      console.error('TicketStoreLocal: Error loading tickets from localStorage', error);
      this.tickets = [];
    }
  }
  ...
  private saveTicketsToStorage(): void {
    try {
      ...
      console.debug('TicketStoreLocal: Saved tickets to localStorage');
    } catch (error) {
      console.error('TicketStoreLocal: Error saving tickets to localStorage:', error);
    }
  }
}