...
export class ScreenShareRequestStoreLocal implements ScreenShareRequestStore {
  ...
  async create(request: Omit<ScreenShareRequest, 'id' | 'createdAt' | 'updatedAt' | 'expiresAt'>): Promise<ScreenShareRequest> {
    ...
    console.debug('ScreenShareRequestStoreLocal: Created request', newRequest.id, 'for ticket', newRequest.ticketId);
    ...
    for (const request of this.requests.values()) {
      if (request.ticketId === ticketId && activeStatuses.includes(request.status)) {
        ...
        if (request.expiresAt && request.expiresAt <= now) {
          console.debug('ScreenShareRequestStoreLocal: Request', request.id, 'has expired');
  ...
  async updateStatus(requestId: string, status: ScreenShareStatus): Promise<ScreenShareRequest | undefined> {
    ...
    if (!request) {
      console.warn('ScreenShareRequestStoreLocal: Request not found for update', requestId);
      return undefined;
    }
    ...
    this.saveRequestsToStorage();
    console.debug('ScreenShareRequestStoreLocal: Updated request', requestId, 'status to', status);
  ...
  async delete(requestId: string): Promise<boolean> {
    ...
    if (deleted) {
      this.saveRequestsToStorage();
      console.debug('ScreenShareRequestStoreLocal: Deleted request', requestId);
    } else {
      console.warn('ScreenShareRequestStoreLocal: Request not found for deletion', requestId);
    }
  ...
  async clear(): Promise<void> {
    this.requests.clear();
    this.saveRequestsToStorage();
    console.debug('ScreenShareRequestStoreLocal: Cleared all requests');
  }
  ...
  reload(): void {
    this.loadRequestsFromStorage();
    console.debug('ScreenShareRequestStoreLocal: Reloaded requests from storage');
  }
  ...
  private loadRequestsFromStorage(): void {
    try {
      ...
      if (stored) {
        ...
        console.debug(`ScreenShareRequestStoreLocal: Loaded ${this.requests.size} requests from localStorage`);
      } else {
        this.requests.clear();
        console.debug('ScreenShareRequestStoreLocal: No requests found in localStorage, initialized empty');
      }
    } catch (error) {
      console.error('ScreenShareRequestStoreLocal: Error loading requests from localStorage', error);
      this.requests.clear();
    }
  }
  ...
  private saveRequestsToStorage(): void {
    try {
      ...
      console.debug(`ScreenShareRequestStoreLocal: Saved ${requestsArray.length} requests to localStorage`);
    } catch (error) {
      console.error('ScreenShareRequestStoreLocal: Error saving requests to localStorage:', error);
    }
  }
