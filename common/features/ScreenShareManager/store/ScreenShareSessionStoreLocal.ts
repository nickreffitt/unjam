...
export class ScreenShareSessionStoreLocal implements ScreenShareSessionStore {
  ...
  async create(session: Omit<ScreenShareSession, 'id' | 'startedAt' | 'lastActivityAt'>): Promise<ScreenShareSession> {
    ...
    console.debug('ScreenShareSessionStoreLocal: Created session', newSession.id, 'for ticket', newSession.ticketId);
  ...
  async update(
    ...
    if (!session) {
      console.warn('ScreenShareSessionStoreLocal: Session not found for update', sessionId);
      return undefined;
    }
    ...
    this.saveSessionsToStorage();
    console.debug('ScreenShareSessionStoreLocal: Updated session', sessionId, 'with', updates);
  ...
  async updateActivity(sessionId: string): Promise<boolean> {
    ...
    if (!session) {
      console.warn('ScreenShareSessionStoreLocal: Session not found for activity update', sessionId);
      return false;
    }
    ...
    console.debug('ScreenShareSessionStoreLocal: Updated activity for session', sessionId);
    return true;
  }
  ...
  async endSession(sessionId: string): Promise<ScreenShareSession | undefined> {
    ...
    if (!session) {
      console.warn('ScreenShareSessionStoreLocal: Session not found for ending', sessionId);
      return undefined;
    }
    ...
    if (updatedSession) {
      console.debug('ScreenShareSessionStoreLocal: Session ended, emitting screenShareSessionEnded event', sessionId);
  ...
  async clear(): Promise<void> {
    this.sessions.clear();
    this.saveSessionsToStorage();
    console.debug('ScreenShareSessionStoreLocal: Cleared all sessions');
  }
  ...
  reload(): void {
    this.loadSessionsFromStorage();
    console.debug('ScreenShareSessionStoreLocal: Reloaded sessions from storage');
  }
  ...
  private loadSessionsFromStorage(): void {
    try {
      ...
      if (stored) {
        ...
        console.debug(`ScreenShareSessionStoreLocal: Loaded ${this.sessions.size} sessions from localStorage`);
      } else {
        this.sessions.clear();
        console.debug('ScreenShareSessionStoreLocal: No sessions found in localStorage, initialized empty');
      }
    } catch (error) {
      console.error('ScreenShareSessionStoreLocal: Error loading sessions from localStorage', error);
      this.sessions.clear();
    }
  }
  ...
  private saveSessionsToStorage(): void {
    try {
      ...
      console.debug(`ScreenShareSessionStoreLocal: Saved ${sessionsArray.length} sessions to localStorage`);
    } catch (error) {
      console.error('ScreenShareSessionStoreLocal: Error saving sessions to localStorage:', error);
    }
  }
